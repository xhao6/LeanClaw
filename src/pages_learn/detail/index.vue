...
// #ifdef MP-WEIXIN
const mdModules = import.meta.glob('/src/pages_learn/static/content/days/*.md', {
  query: '?raw',
  import: 'default',
  eager: true
})
...
const handlePrev = () => {
  ...
  if (prevId > 0) {
    uni.redirectTo({
      url: `/pages_learn/detail/index?day=${prevId}`
    })
  }
}
    ...
    setTimeout(() => {
      uni.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            uni.redirectTo({
              url: `/pages_learn/detail/index?day=${parseInt(dayId.value) + 1}`
            })
          } else {
            // 返回学习页面
            uni.navigateBack()
